...
} else if (window.location.href.includes(config.host)) {
  get('token-info').then(data => {
    console.log(data, data?.token, !data || (data && data.token != Cookie.get('saasToken')), '是否不同步')
    if (!data) {
      if (!window.location.href.includes('verify') && !window.location.href.includes('orgLogin')) {
        window.location.href = '/verify'
      }else{
        set('token-info', {
          token: Cookie.get('saasToken'),
