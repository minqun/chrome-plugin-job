 ...
 * @LastEditTime: 2022-08-13 18:54:56
...
export const checkProduct  = (platform) => {
  ...
  return new Promise((resolve,reject) => {
    ...
    console.log('来检查呀', platform, ['jumia', 'lazada', 'aliexpress'].includes(platform))
    if (['taobao'].includes(platform)) {
      resolve(window.location.href.includes('/item') && window.location.search.includes('id='))
    } else if (['1688'].includes(platform)) {
    resolve( /\/\w{12,13}.html/.test(href) || /\/offer\//.test(href))
    }  else if (['jumia', 'lazada', 'aliexpress'].includes(platform)) {
      resolve(/-\w{0,4}\d{7,}.html/.test(href) || /-\w{8,9}.html/.test(href) || /\/[0-9]{11}.html/.test(href) || /\/s[0-9]{11}.html/.test(href) || /\/[0-9]{14,16}.html/.test(href) || /\/products\/.*/.test(href)) 
    }else if (['shopee'].includes(platform)) {
      let checked = window.location.search && window.location.search.includes('sp_atk=') || window.location.pathname.includes('product') || /i\.\d+\.\d+$/.test(href)
      resolve(checked)
    }
  })
...
export const patPlatform = (host) => /(jumia|lazada|aliexpress|taobao|tmall|1688|xiapibuy|shopee)\./.exec(host||window.location.host)[1]
